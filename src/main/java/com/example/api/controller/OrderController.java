...
import com.example.api.component.OrderResourceAssembler;
import com.example.api.exception.OrderNotFoundException;
import com.example.api.jpa.OrderRepository;
...
public class OrderController {
	...
	@GetMapping("/orders")
	public Resources<Resource<Order>> getAll(){
	...
	public Resource<Order> getById(@PathVariable Long id){
		return assembler.toResource(orderRepository.findById(id).orElseThrow(() -> new OrderNotFoundException(id)));
	}
	
	@PostMapping("/orders")
	public ResponseEntity<Resource<Order>> save(@RequestBody Order order){
		order.setStatus(Status.IN_PROGRESS);
		Order newOrder=orderRepository.save(order);
		
		return ResponseEntity.created(linkTo(methodOn(OrderController.class).getById(order.getId()))
				.toUri()).body(assembler.toResource(newOrder));
	}
	...
	public ResponseEntity<ResourceSupport> cancel(@PathVariable Long id){
		...
		return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
				.body(new VndErrors.VndError("Method not allowed", "You can't cancel an order is in the "+order.getStatus()+" status"));
	}
	...
	public ResponseEntity<ResourceSupport> complete(@PathVariable Long id){
		...
		return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED)
				.body(new VndErrors.VndError("Method not allowed", "You can't complete an order that is in the "+order.getStatus()));
	}
